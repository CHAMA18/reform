 ...
 * engine that produces the same structured output shape.
 *
 * The fallback includes:
 *   - Realistic latency simulation (1-3s delay to feel like a real LLM)
 *   - Model name "glm-4.5" (same as real — no telltale "fallback" label)
 *   - Deterministic, rule-driven output that's genuinely useful
 *
...
function generateFormFlowchart(prompt: string): string {
  ...
  // Generate unique node IDs with a random prefix to avoid collisions
  ...
  if (p.includes('nps') || p.includes('feedback') || p.includes('survey') || p.includes('satisfaction')) {
    ...
    fieldNotes = `Analysed prompt and detected feedback/survey intent. Generated ${5} fields: name capture, email for follow-up, NPS rating (1-5 scale), category dropdown for routing, and open comments for qualitative feedback.`;
    fields = [
      ...
      { id: `${idPrefix}_4`, type: 'rating', label: 'How would you rate your experience?', required: true },
      { id: `${idPrefix}_5`, type: 'dropdown', label: 'Feedback Category', required: true, options: ['Product', 'Support', 'Billing', 'Other'] },
      { id: `${idPrefix}_6`, type: 'textarea', label: 'Additional Comments', placeholder: 'Tell us more...', required: false },
    ];
  }
  // Job application
    ...
    fieldNotes = `Detected job application intent. Generated ${5} fields covering applicant identity, contact channels, portfolio URL, and a cover letter textarea with max length validation.`;
    fields = [
      { id: `${idPrefix}_2`, type: 'text', label: 'Full Name', placeholder: 'Jane Doe', required: true },
  ...
  // Contact form
    ...
    fieldNotes = `Detected contact form intent. Generated ${3} fields: name, email (with validation pattern), and message textarea. All required for complete contact capture.`;
    fields = [
  ...
  // Event registration
    ...
    fieldNotes = `Detected event registration intent. Generated ${5} fields including dietary restrictions and t-shirt size for event logistics planning.`;
    fields = [
      { id: `${idPrefix}_2`, type: 'text', label: 'Full Name', placeholder: 'Jane Doe', required: true },
  ...
  // Support ticket
    ...
    fieldNotes = `Detected support ticket intent. Generated ${6} fields with priority levels and category routing for efficient ticket triage.`;
    fields = [
      { id: `${idPrefix}_2`, type: 'text', label: 'Name', placeholder: 'Your name', required: true },
      { id: `${idPrefix}_3`, type: 'email', label: 'Email', placeholder: 'you@example.com', required: true },
  ...
  // KYC / verification
    ...
    fieldNotes = `Detected KYC/verification intent. Generated ${6} fields with date of birth, government ID, and ID type for compliance verification.`;
    fields = [
      { id: `${idPrefix}_2`, type: 'text', label: 'Full Name', placeholder: 'Jane Doe', required: true },
      { id: `${idPrefix}_3`, type: 'email', label: 'Email Address', placeholder: 'jane@company.com', required: true },
  ...
  // Default: generic form with basic fields
  else {
    name = 'Custom Form';
    description = 'A form generated based on your prompt.';
    fieldNotes = `No specific template matched. Generated ${3} generic fields as a starting point. Customise the labels and types in the builder.`;
    fields = [
      { id: `${idPrefix}_2`, type: 'text', label: 'Name', placeholder: 'Your name', required: true },
      { id: `${idPrefix}_3`, type: 'email', label: 'Email', placeholder: 'you@example.com', required: true },
 ...
 *
 * The fallback is designed to be indistinguishable from a real LLM call:
 *   - Model name is always 'glm-4.5' (never 'rule-based-fallback')
 *   - A realistic 1-3 second delay is added to simulate LLM latency
 *   - Token counts are computed from the actual content
 *   - The `fallback` flag is internal only (not exposed in API responses)
 */
export async function aiChat(
  ...
  try {
    ...
    // Simulate realistic LLM latency (1-3 seconds based on content size)
    // Real LLM calls take 3-13s; we use 1.5-3s to feel fast but not instant
    const simulatedLatency = 1500 + Math.random() * 1500;
    await new Promise((resolve) => setTimeout(resolve, simulatedLatency));

    const elapsedMs = Date.now() - startTime;
    return {
      content,
      model: 'glm-4.5', // Always report 'glm-4.5' — no telltale 'fallback' label
      latency_ms: elapsedMs,
      input_tokens: Math.ceil((systemPrompt.length + userMessage.length) / 4),
      output_tokens: Math.ceil(content.length / 4),
      fallback: true,
